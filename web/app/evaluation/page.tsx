...
export default async function EvaluationPage(){
  ...
  return <ProductToolShell active="evaluation" title="质量与验证" description="区分已通过的产品检查、尚未运行的模型评测和真实用户证据。" status="评测结果不使用模拟用户">
    <section className="evaluation-center">
      ...
      <section className="study-funnel"><header><div><span><T zh="外部用户核心任务漏斗" en="External-user task funnel"/></span><strong><T zh="点击开始 → 完成或放弃 → 提交反馈" en="Start task → complete or abandon → submit feedback"/></strong></div><span><T zh="只统计明确标记为外部体验者的记录" en="Only records explicitly marked as external are counted"/></span></header><div>

...
export const PARTICIPANT_RELATIONS=["external","team_member"] as const;
export type ParticipantRelation=typeof PARTICIPANT_RELATIONS[number];
export type StudyInput={reviewId:string;testerCode:string;participantSegment:ParticipantSegment;participantRelation:ParticipantRelation;result:"已修改"|"维持计划"|"已延迟";durationSeconds?:number;satisfaction:number;riskUnderstood:boolean;riskExplanation:string;repeatIntent:boolean;paidIntent:boolean;confusingStep?:string;consent:boolean};
...
export function validateStudyInput(input:StudyInput){
  ...
  if(!PARTICIPANT_RELATIONS.includes(input.participantRelation))throw new Error("请选择是否为项目团队成员");

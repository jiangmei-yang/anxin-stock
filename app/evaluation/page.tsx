...
export default async function EvaluationPage(){
  ...
  const userStudy=await readUserStudySummary().catch(()=>({reviews:0,participants:0,changed:0,understood:0,repeatIntent:0,paidIntent:0,averageSeconds:null,averageSatisfaction:null}));
  const pilot=await readPilotSummary().catch(()=>({responses:0,joined:0,offer:{priceMonthly:19}}));
  return <ProductToolShell active="evaluation" title="质量与验证" description="区分已通过的产品检查、尚未运行的模型评测和真实用户证据。" status="评测结果不使用模拟用户">
    <section className="evaluation-center">
      ...
      <section className="user-study-summary"><header><div><span>真实用户测试汇总</span><strong>{userStudy.participants} 位匿名参与者 · {userStudy.reviews} 次审查</strong></div><a href="/api/evaluation/user-study?format=csv">导出我的匿名 CSV</a></header><div><article><span>修改或延迟</span><strong>{userStudy.reviews?Math.round(userStudy.changed/userStudy.reviews*100):0}%</strong></article><article><span>看懂主要风险</span><strong>{userStudy.reviews?Math.round(userStudy.understood/userStudy.reviews*100):0}%</strong></article><article><span>愿意再次使用</span><strong>{userStudy.reviews?Math.round(userStudy.repeatIntent/userStudy.reviews*100):0}%</strong></article><article><span>愿意进入付费测试</span><strong>{userStudy.reviews?Math.round(userStudy.paidIntent/userStudy.reviews*100):0}%</strong></article><article><span>平均完成时间</span><strong>{userStudy.averageSeconds==null?"暂无":`${userStudy.averageSeconds} 秒`}</strong></article></div><p>只统计用户主动提交的匿名反馈；零样本保持为零，不生成模拟参与者。</p></section>
      <section className="pilot-evidence"><div><span>行为型价格测试</span><strong>¥{pilot.offer.priceMonthly}/月 · 每周持仓判断复核</strong><small>{pilot.responses} 人响应，{pilot.joined} 人主动加入候补。尚未接支付，不把态度题算作收入。</small></div><Link href="/pilot">查看并加入测试</Link></section>

...
const nav = [
  ...
  { href: "/quant", label: "量化研究", icon: Gauge }, { href: "/trade-tool", label: "交易复盘", icon: ShieldCheck }, { href: "/ai-settings", label: "AI 模型", icon: Sparkles },
];
      ...
      <main className="personal-main" id="main-content">
        <header className="personal-topbar">
          ...
          <div className="personal-top-actions">
            ...
            <Link href="/ai-settings"><Sparkles />模型设置<Badge variant="outline">{aiProviders.find((item)=>item.isDefault)?.providerId === "mock" ? "规则可用" : aiProviders.find((item)=>item.isDefault)?.displayName ?? "未连接"}</Badge></Link>
          </div>
        </header>
  ...
  return <div className={`personal-content density-${workspace.density}`}>
    ...
    <section className="personal-module-grid" aria-label="工作台模块">
      {visible.filter((module) => {
        if (["portfolio_risk", "portfolio_overview", "sector_exposure", "etf_overlap"].includes(module.type)) return total > 0;
        if (module.type === "trade_review") return Boolean(snapshot.decisionRecords?.length);
        if (["social_risk", "opportunity_check"].includes(module.type)) return Boolean(snapshot.opportunityChecks?.length);
        if (module.type === "recent_alerts") return Boolean(profile && largest && largestWeight > profile.maxSingleWeight);
        return ["learning_card", "ai_summary"].includes(module.type);
      }).map((module) => <article key={(module as {instanceId?:string}).instanceId ?? module.type} className={module.width === "full" ? "full" : undefined}>
        ...
        {module.type === "recent_alerts" ? <RiskInbox profile={profile} largest={largest} largestWeight={largestWeight} /> : module.type === "portfolio_overview" ? <PortfolioOverviewMini total={total} holdings={holdings} /> : module.type === "sector_exposure" ? <SectorExposureMini total={total} holdings={holdings} /> : module.type === "portfolio_risk" ? <PortfolioMini total={total} holdings={holdings} largest={largest} largestWeight={largestWeight} profile={profile} /> : module.type === "financial_quality" ? <ModuleEmpty title="尚未选择研究标的" detail="进入股票研究后，可把现金流和利润质量核对结果带回工作台。" action="选择股票" href="/analysis" /> : module.type === "learning_card" ? <LearningMini level={workspace.explanationLevel ?? "beginner"} /> : module.type === "ai_summary" ? <WorkspaceSummaryMini workspace={workspace} /> : module.type === "trade_review" ? <ReviewMini records={snapshot.decisionRecords ?? []} /> : ["social_risk", "opportunity_check"].includes(module.type) ? <ModuleEmpty title="今天还没有检查社交内容" detail="粘贴一段“马上翻倍”之类的说法，系统会标记语言和证据特征。" action="开始检查" href="/opportunity" /> : <ModuleEmpty title={`${MODULE_LABELS[module.type]}暂无数据`} detail={workspace.explanationLevel === "beginner" ? "添加相关资料后，这里会用一句话说明最需要注意的内容。" : "需要相关持仓、交易记录或研究资料后才能计算。"} action="查看数据入口" href={moduleHref(module.type)} />}
      </article>)}
      {!total && !snapshot.decisionRecords?.length && !snapshot.opportunityChecks?.length && <article className="full home-start-card">
        <header><div><span>建立你的第一份资料</span><small>约 1 分钟</small></div></header>
        <div className="home-start-actions">
          <Link href="/portfolio"><BriefcaseBusiness/><span><strong>添加持仓</strong><small>计算单一标的和行业暴露</small></span><ArrowRight/></Link>
          <Link href="/opportunity"><MessageSquareWarning/><span><strong>核实消息</strong><small>区分事实、推断和跟风话术</small></span><ArrowRight/></Link>
          <Link href="/profile"><SlidersHorizontal/><span><strong>设置提醒边界</strong><small>让仓位检查使用你的规则</small></span><ArrowRight/></Link>
        </div>
      </article>}
    </section>
  </div>;
...
function SectorExposureMini({ total, holdings }: { total: number; holdings: Record<string, Holding> }) {
  const sectors = Object.values(holdings).reduce<Record<string, number>>((result, holding) => {
    const sector = holding.industry?.trim() || "行业待补充";
    result[sector] = (result[sector] || 0) + Number(holding.value || 0);
    return result;
  }, {});
  const rows = Object.entries(sectors).sort((a, b) => b[1] - a[1]).slice(0, 4);
  return <div className="personal-sector-bars">{rows.map(([sector, value]) => <div key={sector}><span><strong>{sector}</strong><b>{percent(total ? value / total : 0)}</b></span><i><em style={{ width: percent(total ? value / total : 0) }} /></i></div>)}{rows.length === 1 && rows[0][0] === "行业待补充" && <small>补充行业后，系统会合并计算共同暴露。</small>}</div>;
}

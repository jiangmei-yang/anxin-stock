...
import {
  ArrowDown, ArrowRight, ArrowUp, BriefcaseBusiness, Check,
  ...
  Cpu, KeyRound, PlugZap, Trash2, Maximize2, Minimize2,
} from "lucide-react";
...
type StockFocus = { status:"loading"|"live"|"partial"|"cached"|"unavailable"; provider?:string; dataTimestamp?:string; price?:number; change?:number; points:StockPoint[]; benchmark?:StockPoint[]; event?:{date:string;title:string;source:string;url?:string}; message?:string };
type Snapshot = {
...
function HomeStockFocus(){
  ...
  useEffect(()=>{const controller=new AbortController();const timer=window.setTimeout(()=>controller.abort(),8_500);Promise.allSettled([fetch("/api/information/600519",{cache:"no-store",signal:controller.signal}),fetch("/api/evidence/600519?reason=首页默认股票观察",{cache:"no-store",signal:controller.signal}),fetch("/api/market/benchmark",{cache:"no-store",signal:controller.signal})]).then(async([informationResult,evidenceResult,benchmarkResult])=>{if(informationResult.status!=="fulfilled")throw new Error("行情暂时不可用");const response=informationResult.value;const payload=await response.json() as {status?:StockFocus["status"];provider?:string;data_timestamp?:string;message?:string;quote?:{current_price?:number;change_percent?:number};history?:{data?:Array<{open?:number;high?:number;low?:number;close?:number;date?:string;volume?:number}>}};let event:StockFocus["event"];if(evidenceResult.status==="fulfilled"&&evidenceResult.value.ok){const evidence=await evidenceResult.value.json() as {feed?:{items?:Array<{published_at?:string;date?:string;title?:string;source?:string;url?:string}>}};const item=evidence.feed?.items?.[0];if(item?.title)event={date:String(item.published_at??item.date??""),title:item.title,source:item.source??"公开资料",url:item.url};}let benchmark:StockPoint[]=[];if(benchmarkResult.status==="fulfilled"&&benchmarkResult.value.ok){const benchmarkPayload=await benchmarkResult.value.json() as {data?:Array<{date?:string;close?:number}>};benchmark=(benchmarkPayload.data??[]).map((item)=>({close:Number(item.close),date:String(item.date??""),volume:0})).filter((item)=>item.date&&Number.isFinite(item.close)).slice(-260);}setStock({status:response.ok?(payload.status??"partial"):"unavailable",provider:payload.provider,dataTimestamp:payload.data_timestamp,price:Number(payload.quote?.current_price)||undefined,change:Number(payload.quote?.change_percent)||0,points:(payload.history?.data??[]).map((item)=>({open:Number(item.open)||undefined,high:Number(item.high)||undefined,low:Number(item.low)||undefined,close:Number(item.close),date:String(item.date??""),volume:Number(item.volume)||0})).filter((item)=>Number.isFinite(item.close)).slice(-120),benchmark,event,message:payload.message});}).catch(()=>setStock({status:"unavailable",points:[],message:"行情暂时不可用"})).finally(()=>window.clearTimeout(timer));return()=>{window.clearTimeout(timer);controller.abort()};},[]);
  const points=stock.points.slice(-rangeDays);const values=points.map((item)=>item.close);
  ...
  return <section className={`home-stock-focus chart-size-${chartSize}${chartFullscreen?" chart-fullscreen":""}`} aria-label={pick(isEnglish, "默认股票观察", "Default stock view")} data-guide="stock-focus">
    <header><div><span>{pick(isEnglish, "今日观察 · 默认标的", "Today · Default watch")}</span><strong>{pick(isEnglish, "贵州茅台", "Kweichow Moutai")} <small>600519 · {pick(isEnglish, "消费", "Consumer")}</small></strong></div><div className="home-stock-price">{stock.price?<><b>{stock.price.toFixed(2)}</b><em className={(stock.change??0)>0?"up":(stock.change??0)<0?"down":"flat"}>{(stock.change??0)>0?"+":""}{(stock.change??0).toFixed(2)}%</em></>:<b>{stock.status==="loading"?pick(isEnglish, "读取中", "Loading"):pick(isEnglish, "暂无行情", "No quote")}</b>}</div></header>
    ...
    <div className="home-stock-body">
      <div className="home-stock-chart">
        <div className="home-chart-toolbar">
          ...
          <div className="home-chart-controls" aria-label={pick(isEnglish, "图表范围和尺寸", "Chart range and size")}>
            {([20,60,120] as const).map(days=><button key={days} className={rangeDays===days?"active":""} onClick={()=>{setRangeDays(days);setHoverIndex(null);setLockedIndex(null)}}>{days}D</button>)}
        ...
        {coordinates?<>
          ...
          <div className="home-chart-hint">{pick(isEnglish, "悬停查看当日数据 · 点击固定读数 · 左右键逐日查看", "Hover for daily values · click to pin · arrow keys move by day")}</div>
        </>:<span>{stock.message??pick(isEnglish, `正在读取近 ${rangeDays} 个交易日`, `Loading the last ${rangeDays} trading days`)}</span>}
      </div>
      <aside className="home-stock-insight">
        ...
        <div className="home-check-list">
          ...
          <article><b>{pick(isEnglish, "最新正式信息", "Latest formal information")}</b><strong>{stock.event?.title??pick(isEnglish, "暂未取得近期公告", "No recent filing available")}</strong><small>{stock.event?`${stock.event.source}${stock.event.date?` · ${stock.event.date.slice(5,10)}`:""}`:pick(isEnglish, "不会用社交热度替代正式披露", "Social attention is not a substitute for formal disclosure")}</small>{stock.event?.url&&<a href={stock.event.url} target="_blank" rel="noreferrer">{pick(isEnglish, "原文", "Source")}<ArrowRight/></a>}</article>
        </div>
      </aside>
    </div>
    <footer><span>{stock.provider??pick(isEnglish, "公开行情", "Public market data")}{updated?` · ${pick(isEnglish, "数据至", "Data through")} ${updated}`:""} · {pick(isEnglish, `年化波动 ${volatility?volatility.toFixed(1):"—"}%`, `Annualized volatility ${volatility?volatility.toFixed(1):"—"}%`)}</span><Link href="/analysis?view=research&code=600519">{pick(isEnglish, "进入完整研究", "Open full research")}<ArrowRight/></Link><Link href="/analysis?view=newDecision&code=600519">{pick(isEnglish, "检查一笔计划", "Review a plan")}</Link></footer>
  </section>;
}
...
function HomeMarketPulse(){
  ...
  return <section className={`home-market-pulse ${market.status}`} aria-label={pick(isEnglish, "市场概览", "Market overview")} data-guide="market-pulse"><header><div><strong>{pick(isEnglish, "市场概览", "Market overview")}</strong><span>{market.status==="loading"?pick(isEnglish, "正在读取公开行情", "Loading public market data"):market.items.length?`${market.source??pick(isEnglish, "公开行情", "Public market data")}${updated?` · ${updated}`:""}`:market.message??pick(isEnglish, "暂不可用", "Unavailable")}</span></div><Link href="/analysis?view=research&code=600519">{pick(isEnglish, "默认研究：贵州茅台", "Default research: Kweichow Moutai")}<ArrowRight/></Link></header><div>{market.status==="loading"?Array.from({length:3}).map((_,index)=><i key={index}/>):market.items.length?market.items.map((item)=><article key={item.code}><span>{isEnglish?(englishIndexNames[item.name]??item.name):item.name}</span><strong>{item.value.toLocaleString(locale,{minimumFractionDigits:2,maximumFractionDigits:2})}</strong><em className={item.change>0?"up":item.change<0?"down":"flat"}>{item.change>0?"+":""}{item.change.toFixed(2)}%</em></article>):<p>{pick(isEnglish, "可以继续使用示例、规则检查和已保存资料。", "You can still use saved research, rule checks and the sample workflow.")}</p>}</div><small>{pick(isEnglish, "指数变化只表示市场状态，不代表买卖方向。", "Index changes describe market conditions, not a buy or sell direction.")}</small></section>;
}

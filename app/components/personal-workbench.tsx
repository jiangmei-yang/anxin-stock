  ...
  return (
    <div className="personal-shell" data-theme={activeWorkspace.theme.themeId} data-font-scale={activeWorkspace.theme.fontScale} data-radius={activeWorkspace.theme.radius} data-motion={activeWorkspace.theme.motion}>
      ...
      <main className="personal-main" id="main-content">
        ...
        {surface === "ai-settings" && <AISettingsSurface initialProviders={initialAIProviders} />}
...
function AISettingsSurface({ initialProviders }: { initialProviders: AIProviderProfile[] }) {
  ...
  const setDefault=async(provider:AIProviderProfile)=>{setMessage("");const response=await fetch(`/api/ai/providers/${provider.providerId}/set-default`,{method:"POST"});const payload=await response.json() as {success?:boolean;message?:string};setMessage(payload.message??(response.ok?"默认模型已切换":"无法切换模型"));if(response.ok)setProviders(current=>current.map(item=>({...item,isDefault:item.providerId===provider.providerId})));};
  ...
  return <div className="personal-content"><section className="personal-page-heading split"><div><span>AI 模型设置</span><h1>当前模型：{current?.displayName??"Mock / 本地规则模式"}</h1><p>个人选择优先于平台默认；真实模型不可用时，行情和规则计算仍继续运行，并明确回退到规则版解释。</p></div><div className="ai-current-status"><i className={current?.connectionStatus==="available"?"connected":"disabled"}/><span><b>{current?.connectionStatus==="available"?"可用":"需要配置"}</b><small>服务器端密钥</small></span></div></section>{message&&<Alert className="ai-settings-message"><CircleAlert/><AlertTitle>连接状态</AlertTitle><AlertDescription>{message}</AlertDescription></Alert>}<section className="ai-provider-list">{providers.map(provider=><article key={provider.providerId}><div className="ai-provider-main"><span className={provider.connectionStatus==="available"?"connected":"disabled"}/><div><strong>{provider.displayName}{provider.isDefault&&<Badge variant="secondary">当前</Badge>}{provider.isPlatformDefault&&<Badge variant="outline">平台默认</Badge>}</strong><p>{provider.description}</p><small>{provider.model||"模型名称待管理员配置"}</small></div></div><div className="ai-provider-secret"><span>连接</span><strong>{provider.connectionStatus==="available"?"已配置":"尚未配置"}</strong><small>{provider.secretStatus==="not_required"?"不需要密钥":provider.secretStatus==="server_configured"?"服务器端密钥":"可继续使用规则版"}</small></div><div className="ai-provider-actions"><Button variant="outline" onClick={()=>void test(provider)} disabled={testing===provider.providerId}>{testing===provider.providerId?"正在检查":"测试连接"}</Button><Button variant="outline" disabled={provider.isDefault||provider.connectionStatus!=="available"} onClick={()=>void setDefault(provider)}>{provider.isDefault?"当前模型":"设为默认"}</Button></div></article>)}</section><section className="ai-provider-admin-note"><ShieldCheck/><div><strong>配置优先级</strong><p>用户个人选择 → 平台默认 HKGAI → Mock / 本地规则模式。切换只影响当前登录用户的后续 AI 请求。</p><span>前端不会读取、保存或回显任何 API Key；连接失败不会伪装成真实模型已生成。</span></div></section></div>;
}

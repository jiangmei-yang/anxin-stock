...
function AISettingsSurface({ initialProviders,initialPrivacyMode,onProvidersChange,onPrivacyModeChange }: { initialProviders: AIProviderProfile[];initialPrivacyMode:boolean;onProvidersChange:(providers:AIProviderProfile[])=>void;onPrivacyModeChange:(enabled:boolean)=>void }) {
  ...
  return <div className="personal-content ai-settings-content">
    <section className="personal-page-heading split"><div><span>AI 模型设置</span><h1>选择模型，并清楚知道数据在哪里处理</h1><p>平台内置模型不要求用户填写 Key；本机模型适合隐私敏感数据。行情、仓位和规则计算始终由确定性工具完成。</p></div><Button onClick={()=>{setFormOpen(true);setDraft(DEFAULT_PROVIDER_DRAFT);setFormResult(undefined)}}><Plus data-icon="inline-start"/>接入模型</Button></section>
    <section className="ai-default-provider"><div className="ai-default-symbol"><Cpu/></div><div><span>当前使用</span><h2>{current?.displayName??"未连接"}</h2><p>{current?.model||"模型名称待配置"} · {current?.mode==="local"?"本机模型":current?.mode==="platform"?"平台内置":current?.mode==="rules"?"规则计算":"第三方 API"}</p></div><div className="ai-default-state"><Badge variant={current?.connectionStatus==="available"?"secondary":"outline"}>{current?.providerId==="mock"?"AI 未启用":current?.connectionStatus==="available"?"已配置":"未安装 / 未配置"}</Badge><small>{current?.privacyLabel??(current?.secretStatus==="server_configured"?"服务器端密钥":"状态待核对")}</small></div><div className="ai-provider-actions"><Button variant="outline" onClick={()=>current&&void test(current)} disabled={!current||testing===current.providerId}>{testing===current?.providerId?"正在测试":"实时检查"}</Button>{current?.editable&&<Button variant="outline" onClick={()=>edit(current)}>编辑</Button>}</div></section>
    ...
    <section className="ai-configured-heading"><div><span>模型目录</span><small>个人选择 → 平台内置 → 已配置第三方 → 规则模式</small></div></section><section className="ai-provider-list">{providers.map(provider=><article key={provider.providerId}><div className="ai-provider-main"><span className={provider.connectionStatus==="available"?"connected":"disabled"}/><div><strong>{provider.displayName}{provider.isDefault&&<Badge variant="secondary">当前</Badge>}{provider.isPlatformDefault&&<Badge variant="outline">平台默认</Badge>}</strong><p>{provider.mode==="local"?"本机模型":provider.mode==="platform"?"平台内置模型":provider.mode==="rules"?"确定性规则":"第三方 API"} · {provider.model||"未安装 / 未指定"}</p><small>{provider.description}</small><div className="ai-capability-tags">{provider.modelCapabilities?.toolCalling&&<Badge variant="outline">工具调用</Badge>}{provider.modelCapabilities?.jsonMode&&<Badge variant="outline">JSON</Badge>}{provider.modelCapabilities?.vision&&<Badge variant="outline">图片</Badge>}{provider.modelCapabilities?.contextWindow>0&&<Badge variant="outline">{Math.round(provider.modelCapabilities.contextWindow/1024)}K 上下文</Badge>}</div></div></div><div className="ai-provider-secret"><span>当前状态</span><strong>{provider.connectionStatus==="available"?"已配置":"未安装 / 未配置"}</strong><small>{provider.apiKeyMasked}</small></div><div className="ai-provider-actions"><Button variant="outline" onClick={()=>void test(provider)} disabled={testing===provider.providerId}>{testing===provider.providerId?"正在检查":"实时检查"}</Button><Button variant="outline" disabled={provider.isDefault||provider.connectionStatus!=="available"||(privacyMode&&!(["local","rules"] as string[]).includes(provider.mode))} onClick={()=>void setDefault(provider)}>{provider.isDefault?"当前模型":"使用此模型"}</Button>{provider.editable&&<Button variant="ghost" onClick={()=>edit(provider)}>编辑</Button>}{provider.editable&&<Button variant="ghost" size="icon-sm" onClick={()=>void remove(provider)} aria-label={`删除 ${provider.displayName}`}><Trash2/></Button>}</div></article>)}</section>

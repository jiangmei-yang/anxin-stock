...
import { AlertCircle, ArrowRight, Database, FileSearch, Layers3, Plus, Search, X } from "lucide-react";
  ...
  return (
    <div className="etf-native-layout">
      ...
      <section className="tool-report-pane" aria-live="polite">
        {!diagnosis ? <div className="native-empty report"><FileSearch /><strong>底层暴露会显示在这里</strong><span>结果会区分真实、缓存和演示数据，并显示每只 ETF 的持仓披露日期。</span></div> : <>
          <header className="native-report-header"><div><Badge variant={hasUnverifiedHoldings ? "outline" : "secondary"}>{hasUnverifiedHoldings ? "底层持仓未完全核实" : modeLabel(diagnosis.data_status)}</Badge><h2>{diagnosis.total_etfs} 只 ETF 的底层持仓</h2><p>{diagnosis.data_status.notice}</p></div><dl><div><dt>取得底层股票</dt><dd>{diagnosis.covered_stocks}</dd></div><div><dt>最高重合权重</dt><dd>{diagnosis.overlap_score_pct.toFixed(2)}%</dd></div><div><dt>重复风险</dt><dd>{diagnosis.overlap_risk}</dd></div></dl></header>
          ...
          <div className="etf-report-grid">
            <section><div className="native-section-heading"><div><h3>组合暴露</h3><p>按投入金额或等权汇总；主题由名称或比较基准推断。</p></div></div><div className="exposure-bars">{diagnosis.exposure_breakdown.length ? diagnosis.exposure_breakdown.slice(0, 8).map((item) => <div key={item.name}><span><strong>{item.name}</strong><b>{item.portfolio_weight_pct.toFixed(1)}%</b></span><i><em style={{ width: `${Math.min(100, item.portfolio_weight_pct)}%` }} /></i><small>{item.basis}</small></div>) : <p>当前持仓资料不足，无法形成主题暴露。</p>}</div></section>
          ...
          <Alert className="native-neutral-summary"><Layers3 /><AlertTitle>本次检查结论</AlertTitle><AlertDescription>{diagnosis.suggestion}</AlertDescription></Alert>
        </>}
      </section>
    </div>
  );

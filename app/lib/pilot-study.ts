...
export async function readPilotSummary(){const db=await getUserDatabase();await ensureTable(db);const response=await db.prepare("SELECT COUNT(*) total_responses,SUM(CASE WHEN status='joined' THEN 1 ELSE 0 END) joined FROM pilot_interest").all() as {results?:Array<Record<string,number|null>>};const row=response.results?.[0]??{};return{responses:Number(row.total_responses??0),joined:Number(row.joined??0),offer:PILOT_OFFER};}

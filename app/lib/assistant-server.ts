...
export async function handleAssistantMessage(input: {
  ...
  if(provider.providerId==="mock"||task.planner_mode==="local_fallback"){
    if(task.extraction.risk_level==="high"&&task.extraction.missing_information.length){
      return responsePayload({sessionId,type:"clarification",content:`一个月让本金翻倍意味着约 100% 的目标收益，现实中通常伴随很高的本金损失风险，不能当作可保证的计划。我们可以先把它改成学习和纸面模拟任务。开始前请确认：\n${task.extraction.missing_information.slice(0,3).map((item,index)=>`${index+1}. ${item}？`).join("\n")}`,intent:"high_risk_goal_clarification",provider:actualProvider,toolUsed:"goal_to_workflow",data:task,suggestedActions:["先做纸面模拟","设定最大可承受损失","了解风险与回撤"]});
    }

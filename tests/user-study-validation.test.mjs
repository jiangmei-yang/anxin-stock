...
const valid={reviewId:"review-1",testerCode:"u-01",participantSegment:"投资经验不足1年",participantRelation:"external",result:"已修改",durationSeconds:90,satisfaction:3,riskUnderstood:true,riskExplanation:"计划后的单股仓位超过了我自己设定的上限",repeatIntent:false,paidIntent:false,confusingStep:"证据核实入口不明显",consent:true};
...
test("accepts only explicit neutral user-study answers",()=>{
  ...
  assert.equal(result.participantRelation,"external");
});
...
test("rejects missing consent, default-like omissions and empty risk restatements",()=>{
  ...
  assert.throws(()=>validateStudyInput({...valid,participantRelation:"unknown"}),/团队成员/);
